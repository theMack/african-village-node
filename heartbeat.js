...
import { getCurrentChannel, getCurrentFrequency, getSignalStrength } from './spectrum.js'
...
export async function beat() {
  ...
  if (success) {
    consecutiveFaults = 0
    logger.debug('heartbeat', `Beat — peers: ${meshPeers}, ch: ${telemetry.tvwsChannel}, uptime: ${telemetry.uptimeSeconds}s`)

    // Write detailed telemetry every 5 beats
    if (Math.floor(process.uptime() / (config.timing.heartbeatMs / 1000)) % 5 === 0) {
      await writeTelemetry(telemetry)
    }
  } else {
    consecutiveFaults++
    logger.warn('heartbeat', `Fault ${consecutiveFaults}/${MAX_FAULTS_BEFORE_ALERT}`)
...
export function startHeartbeat() {
  ...
  logger.info('heartbeat', `Starting — interval: ${config.timing.heartbeatMs}ms`)

  // Beat immediately on start
  beat()
