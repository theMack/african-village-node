...
async function boot() {
  ...
  if (!registered) {
    logger.warn(
      "agent",
      "Node registration failed — continuing with degraded telemetry",
    );
  }
  ...
  if (!channels.length) {
    logger.error(
      "agent",
      "No TVWS channels available — cannot proceed with transmission",
    );
    await dispatchSignal(
      "village:infrastructure:node",
      "node:fault:no_channels",
      {
    ...
    logger.info(
      "agent",
      `Selected channel ${optimal.channel} (${optimal.frequencyMhz} MHz)`,
    );
    ...
    if (!radioConfigured) {
      logger.warn(
        "agent",
        "Radio configuration failed — node will not transmit TVWS",
      );
    }
  }
  ...
  logger.info(
    "agent",
    `Scheduling spectrum refresh every ${config.fcc.queryIntervalMs / 60000}min`,
  );
...
process.on("uncaughtException", async (err) => {
  logger.error("agent", "Uncaught exception", {
    error: err.message,
    stack: err.stack,
  });
